#!/usr/bin/env node
/**
 * Alibaba DashScope API 本地测试脚本 (OpenAI 兼容模式)
 *
 * 用法:
 *   node scripts/test-alibaba.mjs <图片路径>
 *
 * 示例:
 *   node scripts/test-alibaba.mjs ~/Desktop/test.jpg
 *   node scripts/test-alibaba.mjs ./test.png
 *   node scripts/test-alibaba.mjs scripts/1.jpg
 */
...
async function main() {
  ...
  if (!imgPath) {
    console.log(`
${c.bold}Alibaba DashScope API 本地测试脚本 (OpenAI 兼容模式)${c.reset}

用法: node scripts/test-alibaba.mjs <图片路径>

示例:
  node scripts/test-alibaba.mjs ~/Desktop/test.jpg
  node scripts/test-alibaba.mjs ./assets/sample.png
  node scripts/test-alibaba.mjs scripts/1.jpg
`)
    process.exit(1)
  }
  ...
  console.log(`${c.bold}        Alibaba DashScope API 连接测试${c.reset}`)
