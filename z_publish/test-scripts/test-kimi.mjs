 ...
 *
 * 用法:
 *   node scripts/test-kimi.mjs <图片路径>
 *
 * 示例:
 *   node scripts/test-kimi.mjs ~/Desktop/test.jpg
 *   node scripts/test-kimi.mjs ./test.png
 *   node scripts/test-kimi.mjs scripts/1.jpg
 */
...
function loadEnv() {
  ...
  if (!apiKey) {
    log('错误', '.env.local 中未找到 KIMI_API_KEY', c.red)
    process.exit(1)
  }
...
async function main() {
  ...
  if (!imgPath) {
    console.log(`
${c.bold}Kimi API 本地测试脚本${c.reset}

用法: node scripts/test-kimi.mjs <图片路径>

示例:
  node scripts/test-kimi.mjs ~/Desktop/test.jpg
  node scripts/test-kimi.mjs ./assets/sample.png
  node scripts/test-kimi.mjs scripts/1.jpg
`)
    process.exit(1)
  }
  ...
  console.log(`${c.bold}        Kimi API 连接测试${c.reset}`)
...
请直接返回JSON数组，不要有其他内容。`
  ...
  log('请求', `模型: ${requestBody.model}`, c.cyan)
  ...
  try {
    ...
    console.log(`\n${c.bold}${'='.repeat(60)}${c.reset}`)
    console.log(`${c.green}${c.bold}测试通过！Kimi API 连接正常。${c.reset}`)
    console.log(`${c.bold}${'='.repeat(60)}\n`)
    ...
    if (error.name === 'AbortError') {
      ...
      console.log('  2. api.kimi.com 无法访问')
      console.log('  3. 图片过大，传输时间过长')
    } else if (error.cause?.code === 'UND_ERR_CONNECT_TIMEOUT') {
      log('错误', '底层连接超时', c.red)
      console.log(`\n${c.yellow}可能原因：${c.reset}`)
      console.log('  1. 本地网络无法访问 api.kimi.com')
